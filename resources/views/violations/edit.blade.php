<x-app-layout>
    @section('header', 'Edit Violation Type')
            ...
            <div class="relative flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div class="flex flex-col sm:flex-row sm:items-center gap-5">
                    ...
                    <div>
                        ...
                        <h1 class="text-3xl font-extrabold text-white tracking-tight">Edit Violation Type</h1>
                        <p class="text-indigo-100/70 text-sm mt-2 max-w-xl leading-relaxed">Updating: <span class="text-white font-semibold">{{ $violation->code }}</span> — {{ $violation->title }}</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="p-8 space-y-8">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="code" value="{{ old('code', $violation->code) }}" placeholder="e.g. V-001" required 
                            ...
                            <div class="relative">
                                ...
                                <select name="category" required 
                                    ...
                                    @foreach(['Academic', 'Behavioral', 'Administrative', 'Security', 'General'] as $cat)
                                        <option value="{{ $cat }}" {{ old('category', $violation->category) == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                                    @endforeach
                                </select>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="title" value="{{ old('title', $violation->title) }}" placeholder="Official violation title..." required 
                                class="w-full pl-10 pr-4 py-3 bg-gray-50/50 border border-gray-200 text-slate-800 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all placeholder-gray-400">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4" placeholder="Define the standard sanction for this violation..." required 
                            class="w-full px-4 py-3 bg-gray-50/50 border border-gray-200 text-slate-800 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all resize-none placeholder-gray-400">{{ old('description', $violation->description) }}</textarea>
                    </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            @foreach(['Minor', 'Major'] as $level)
                                <label class="cursor-pointer group">
                                    <input type="radio" name="severity" value="{{ $level }}" class="hidden peer" {{ old('severity', $violation->severity) == $level ? 'checked' : '' }}>
