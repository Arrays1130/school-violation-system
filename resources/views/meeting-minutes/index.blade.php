<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto space-y-8">
        ...
        {{-- Modern Header --}}
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-slate-900 to-indigo-950 px-6 py-5 shadow-xl shadow-indigo-900/10 border border-indigo-900/20" x-data="{ uploadModal: false }">
            ...
            <div class="relative flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div>
                    <div class="inline-flex items-center gap-2 px-2.5 py-1 rounded-full bg-white/10 border border-white/10 text-white/80 text-[10px] font-bold uppercase tracking-widest mb-1.5 backdrop-blur-md">
                        <i data-lucide="folder-git" class="w-3.5 h-3.5 text-indigo-400 animate-pulse"></i>
                        Minutes & Documents
                    </div>
                    <h2 class="text-xl sm:text-2xl font-bold text-white tracking-tight">Document Repository</h2>
                    <p class="text-indigo-100/70 text-xs mt-1 leading-relaxed">Manage external attachments, documentary evidence, and recorded minutes.</p>
                </div>
                <div class="flex flex-wrap items-center gap-3">
                    <button @click="uploadModal = true" class="inline-flex items-center justify-center gap-2 px-4 py-2 bg-indigo-600 rounded-xl text-sm font-bold text-white hover:bg-indigo-700 shadow-md shadow-indigo-600/20 hover:shadow-lg transition-all duration-200 hover:-translate-y-0.5">
                        <i data-lucide="upload-cloud" class="w-4 h-4"></i>
                        <span>Upload Document</span>
                    </button>
                    <a href="{{ route('meeting-minutes.create') }}" class="inline-flex items-center justify-center gap-2 px-4 py-2 bg-white/10 border border-white/10 rounded-xl text-sm font-bold text-white hover:bg-white/20 shadow-sm backdrop-blur-md transition-all duration-200 hover:-translate-y-0.5">
            ...
            <div class="mt-5 pt-5 border-t border-white/10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="relative overflow-hidden rounded-xl border border-white/5 bg-white/5 p-4 backdrop-blur-md flex items-center justify-between group hover:bg-white/10 transition-all duration-250">
                        <div>
                            <span class="text-[10px] font-bold text-indigo-200/60 uppercase tracking-widest">Total Files</span>
                            <div class="text-2xl font-black text-white mt-1">{{ $totalFiles }}</div>
                        </div>
                        <div class="w-10 h-10 rounded-xl bg-white/10 text-indigo-300 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform">
                            <i data-lucide="files" class="w-5 h-5"></i>
                        </div>
                    </div>
                    <div class="relative overflow-hidden rounded-xl border border-indigo-500/20 bg-gradient-to-br from-indigo-500/10 to-indigo-950/30 p-4 flex items-center justify-between group hover:bg-indigo-500/20 transition-all duration-250">
                        <div>
                            <span class="text-[10px] font-bold text-indigo-300/80 uppercase tracking-widest">PDF Documents</span>
                            <div class="text-2xl font-black text-white mt-1">{{ $pdfFiles }}</div>
                        </div>
                        <div class="w-10 h-10 rounded-xl bg-indigo-500/20 text-indigo-200 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform">
                            <i data-lucide="file-text" class="w-5 h-5"></i>
                        </div>
                    </div>
                    <div class="relative overflow-hidden rounded-xl border border-white/5 bg-white/5 p-4 backdrop-blur-md flex items-center justify-between group hover:bg-white/10 transition-all duration-250">
                        <div>
                            <span class="text-[10px] font-bold text-indigo-200/60 uppercase tracking-widest">Total Storage</span>
                            <div class="text-2xl font-black text-white mt-1">{{ $totalSizeMB }} <span class="text-sm font-semibold text-indigo-300">MB</span></div>
                        </div>
                        <div class="w-10 h-10 rounded-xl bg-white/10 text-indigo-300 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform">
                        ...
                        <div class="p-8 text-left">
                            <div class="flex items-center justify-between mb-6">
                                <div class="flex items-center gap-3">
                                    ...
                                    <h3 class="text-lg font-bold text-slate-800 tracking-tight">Upload Document</h3>
                                </div>
                                <button type="button" @click="uploadModal = false" class="w-8 h-8 rounded-lg bg-gray-50 hover:bg-gray-100 text-slate-400 hover:text-gray-600 flex items-center justify-center transition-colors">
                                    <i data-lucide="x" class="w-5 h-5"></i>
                                </button>
                            </div>
                            <div class="space-y-5">
                                <div>
                                    <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Link to Case (Optional)</label>
                                    <div class="relative">
                                ...
                                <div>
                                    <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Select File</label>
                                    <input type="file" name="file" class="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-600 hover:file:bg-indigo-150 transition-all" required>
                                </div>
                                <div>
                                    <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Display Name / Label</label>
                                    <div class="relative">
        ...
        <div class="bg-white/90 backdrop-blur-xl rounded-2xl ring-1 ring-slate-100/80 shadow-[0_4px_20px_rgb(0,0,0,0.03)] overflow-hidden p-8 space-y-6">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-100 text-left">
                        <thead class="bg-slate-50/75">
                            <tr>
                                <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">File Name</th>
                                <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Linked Case</th>
                                <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Uploaded By</th>
                        ...
                        <tbody class="bg-transparent divide-y divide-slate-50">
                            @forelse($records as $record)
                                <tr class="hover:bg-indigo-50/30 transition-all duration-150">
                                    <td class="px-6 py-4.5 whitespace-nowrap">
                                        <div class="flex items-center gap-3.5">
                                            <div class="w-9 h-9 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center shrink-0 shadow-sm border border-indigo-100/50">
                                    ...
                                    <td class="px-6 py-4.5 whitespace-nowrap text-right text-sm">
                                        <div class="flex items-center justify-end gap-3">
                                            @if($record->view_url)
                                                <a href="{{ $record->view_url }}" class="inline-flex items-center gap-1 px-2.5 py-1 bg-gray-50 hover:bg-gray-100 border border-gray-200 text-xs font-bold text-slate-700 rounded-lg transition-colors">
                                                    <i data-lucide="eye" class="w-3.5 h-3.5"></i>
                                                    View
                                                </a>
                                            @endif
                                            @if($record->download_url)
                                                <a href="{{ $record->download_url }}" class="inline-flex items-center gap-1 px-2.5 py-1 bg-indigo-50 hover:bg-blue-100 border border-indigo-100 text-xs font-bold text-indigo-700 rounded-lg transition-colors">
