<x-app-layout>
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="flex items-center gap-2 text-sm text-slate-500 mb-6">
            <a href="{{ route('dashboard') }}" class="hover:text-slate-800 dark:hover:text-gray-300 transition-colors">Dashboard</a>
            <i data-lucide="chevron-right" class="w-4 h-4"></i>
            <a href="{{ route('students.index') }}" class="hover:text-slate-800 dark:hover:text-gray-300 transition-colors">Students</a>
            <i data-lucide="chevron-right" class="w-4 h-4"></i>
            <span class="text-slate-800 dark:text-white font-medium">Import Students</span>
        </div>

        <div class="max-w-2xl mx-auto">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                    <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Import Students</h1>
                            ...
                            <div class="flex items-center justify-center w-full">
                                <label for="dropzone-file" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600 transition-colors">
                                    <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                        ...
                                        <p class="text-xs text-slate-500 dark:text-slate-400">XLSX, XLS or CSV</p>
                                    </div>
                                    <input id="dropzone-file" type="file" name="file" class="hidden" accept=".xlsx,.xls,.csv" required />
                                </label>
                            </div>
                            <div id="file-name" class="mt-2 text-sm text-gray-600 dark:text-slate-400 text-center hidden"></div>
                        </div>
