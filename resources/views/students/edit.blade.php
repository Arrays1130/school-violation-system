    ...
    <div class="max-w-3xl mx-auto space-y-8">
        ...
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-slate-900 to-indigo-950 px-6 py-5 shadow-xl shadow-indigo-900/10 border border-indigo-900/20">
            ...
            <div class="relative flex items-center gap-5">
                <a href="{{ route('students.show', $student) }}" class="w-10 h-10 rounded-xl bg-white/10 border border-white/10 flex items-center justify-center text-white/80 hover:text-white hover:bg-white/20 transition-all shadow-sm backdrop-blur-md hover:-translate-x-0.5">
                    <i data-lucide="arrow-left" class="w-4 h-4"></i>
                </a>
                <div>
                    <h2 class="text-2xl font-bold text-white tracking-tight">Edit Student Profile</h2>
                    <p class="text-indigo-100/70 text-xs mt-1">Updating Profile: <span class="text-white font-medium">{{ $student->full_name }}</span></p>
                </div>
            </div>
        </div>
                                ...
                                <div class="relative">
                                    <select name="section" required 
                                        ...
                                        @foreach(['A', 'B', 'C'] as $sec)
                                            <option value="{{ $sec }}" {{ old('section', $student->section) == $sec ? 'selected' : '' }}>Section {{ $sec }}</option>
                                        @endforeach
                                    </select>
                                    <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    @php
                                        ...
                                        // Ensure the student's current academic year is in the list even if it's outside the range
                                        if ($student->academic_year && !in_array($student->academic_year, $years)) {
                                            $years[] = $student->academic_year;
                                            sort($years);
                                        }
                                    @endphp
                                    <select name="academic_year" required 
                                        ...
                                        @foreach($years as $year)
                                            <option value="{{ $year }}" {{ old('academic_year', $student->academic_year) == $year ? 'selected' : '' }}>
                                                {{ $year }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <p class="text-xs text-slate-500 mt-1.5">Changing this will update the student's listed academic year.</p>
                            </div>
                            ...
                            <div class="relative">
                                <select name="department" required 
                                    ...
                                    @foreach([
                                        ...
                                        <option value="{{ $value }}" {{ old('department', $student->department) == $value ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                                <div class="absolute inset-y-0 right-0 pr-3.5 flex items-center pointer-events-none text-slate-400">
                                    <i data-lucide="chevron-down" class="w-4.5 h-4.5"></i>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="email" name="guardian_email" value="{{ old('guardian_email', $student->guardian_email) }}" placeholder="e.g. guardian@example.com"
                                        class="w-full pl-10 pr-4 py-3 bg-gray-50/50 border border-gray-200 rounded-xl text-sm font-medium text-slate-800 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all placeholder-gray-400">
                                </div>
                            </div>
                        </div>
