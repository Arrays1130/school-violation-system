<x-app-layout>
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="mb-8">
            ...
            <p class="text-gray-600 dark:text-gray-300 mt-2">Recover deleted students or individual cases.</p>
        </div>
                ...
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
                            ...
                            <th class="px-6 py-4 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Department</th>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors group">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-sm font-semibold text-gray-500 dark:text-gray-400">
                                            {{ $student->initials }}
                                        </div>
                                        <div>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <form id="restore-student-{{ $student->id }}" action="{{ route('students.restore', $student->id) }}" method="POST" class="inline-block">
                                            ...
                                            <button type="button" 
                                                    onclick="confirmAction('restore-student-{{ $student->id }}', {
                                                        ...
                                                        text: 'This will bring the student and all their disciplinary records back to the active list.',
                                                        icon: 'info',
                                        ...
                                        <form id="force-delete-student-{{ $student->id }}" action="{{ route('students.force-delete', $student->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" onclick="confirmAction('force-delete-student-{{ $student->id }}', {title: 'Permanently Delete Student?', text: 'This cannot be undone. The student and all records will be erased forever.', icon: 'warning', confirmButtonText: 'Yes, delete permanently', confirmButtonColor: '#dc2626'})" class="inline-flex items-center gap-2 px-4 py-2 bg-red-50 hover:bg-red-100 text-red-700 dark:bg-red-900/30 dark:hover:bg-red-900/50 dark:text-red-400 rounded-lg text-sm font-bold transition-all border border-red-100 dark:border-red-800">
                                ...
                                <td colspan="4" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        <div class="w-20 h-20 bg-gray-50 dark:bg-gray-700/50 rounded-full flex items-center justify-center mb-4 text-gray-300">
                                            <i data-lucide="trash" class="w-10 h-10"></i>
                                        </div>
                                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">Trash is empty</h3>
                                        <p class="text-sm text-gray-500 dark:text-gray-400 max-w-sm">
                                            No deleted students were found. Students you delete will appear here for recovery.
                                        </p>
                                    </div>
                                </td>
                            </tr>
