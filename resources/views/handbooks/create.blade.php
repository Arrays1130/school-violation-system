<x-app-layout>
    @section('header', 'Add New Handbook Entry')
    ...
    <div class="max-w-3xl mx-auto space-y-8">
        ...
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-slate-900 to-indigo-950 px-6 py-5 shadow-xl shadow-indigo-900/10 border border-indigo-900/20">
            ...
            <div class="relative flex items-center gap-5">
                <a href="{{ route('handbooks.index') }}" class="w-10 h-10 rounded-xl bg-white/10 border border-white/10 flex items-center justify-center text-white/80 hover:text-white hover:bg-white/20 transition-all shadow-sm backdrop-blur-md hover:-translate-x-0.5">
                ...
                <div>
                    ...
                    <h2 class="text-2xl font-bold text-white tracking-tight">Add Handbook Entry</h2>
                    <p class="text-indigo-100/70 text-xs mt-1">Publish new institutional rules and regulation guidelines.</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white/90 backdrop-blur-xl rounded-2xl ring-1 ring-slate-100/80 shadow-[0_4px_20px_rgb(0,0,0,0.03)] overflow-hidden">
            <form action="{{ route('handbooks.store') }}" method="POST">
                @csrf
                <div class="p-8 space-y-8">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Title *</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                                    <i data-lucide="type" class="w-4.5 h-4.5"></i>
                                </div>
                                <input type="text" name="title" value="{{ old('title') }}" placeholder="e.g. Student Code of Conduct" required autofocus
                        ...
                        <div>
                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Content & Description *</label>
                            <textarea name="content" rows="8" required placeholder="Describe the rule in detail..."
                                class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-medium text-gray-950 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all placeholder-gray-400 leading-relaxed">{{ old('content') }}</textarea>
                            @error('content') <p class="text-rose-500 text-xs mt-1 font-semibold">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">External Document Link (URL)</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                                    <i data-lucide="link" class="w-4.5 h-4.5"></i>
                                </div>
                                <input type="url" name="attachment" value="{{ old('attachment') }}" placeholder="https://example.com/policy.pdf" 
                                    class="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-medium text-gray-950 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all placeholder-gray-400">
                            </div>
                            <p class="text-xs text-slate-400 mt-2 font-medium">Optional: Paste a link to a Google Doc or hosted PDF document.</p>
                ...
                <div class="px-8 py-5 bg-gray-50 border-t border-gray-150 flex items-center justify-end gap-3">
                    <a href="{{ route('handbooks.index') }}" class="px-4 py-2 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-700 hover:bg-gray-50 transition-all">
                        Cancel
                    </a>
