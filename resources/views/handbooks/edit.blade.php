    ...
    <div class="max-w-3xl mx-auto space-y-8">
        ...
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-slate-900 to-indigo-950 px-6 py-5 shadow-xl shadow-indigo-900/10 border border-indigo-900/20">
            ...
            <div class="relative flex items-center gap-5">
                <a href="{{ route('handbooks.show', $handbook) }}" class="w-10 h-10 rounded-xl bg-white/10 border border-white/10 flex items-center justify-center text-white/80 hover:text-white hover:bg-white/20 transition-all shadow-sm backdrop-blur-md hover:-translate-x-0.5">
                ...
                <div>
                    ...
                    <h2 class="text-2xl font-bold text-white tracking-tight">Edit Policy Entry</h2>
                    <p class="text-indigo-100/70 text-xs mt-1">Updating Entry: <span class="text-white font-medium">{{ $handbook->title }}</span></p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white/90 backdrop-blur-xl rounded-2xl ring-1 ring-slate-100/80 shadow-[0_4px_20px_rgb(0,0,0,0.03)] overflow-hidden">
            <form action="{{ route('handbooks.update', $handbook) }}" method="POST">
                ...
                <div class="px-8 py-5 bg-gray-50 border-t border-gray-150 flex items-center justify-end gap-3">
                    <a href="{{ route('handbooks.show', $handbook) }}" class="px-4 py-2 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-700 hover:bg-gray-50 transition-all">
                        Cancel
                    </a>
