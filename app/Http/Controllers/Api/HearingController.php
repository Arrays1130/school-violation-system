...
class HearingController extends Controller
{
    public function upcoming(Request $request)
    {
        ...
        $studentIds = Student::query()
            ->when($user->isDean() && ! $user->isSuperAdmin(), function ($query) use ($user) {
                $longName = Student::resolveDepartmentLongName($user->department);
                $query->where('department', $longName);
            })
            ->pluck('id');
    ...
    public function calendar(Request $request)
    {
        ...
        if ($user->isDean() && ! $user->isSuperAdmin()) {
            $longName = DepartmentResolver::shortcutToLong($user->department);
